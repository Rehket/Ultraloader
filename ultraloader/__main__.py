...
def login(
    ...
    if json_out:
        ...
        credential_directory = Path(ULTRALOADER_CREDENTIAL_DIRECTORY)
        if not credential_directory.exists():
            credential_directory.mkdir(parents=True)
        with open(Path(ULTRALOADER_CREDENTIAL_FILE_PATH), "w") as creds:
            creds.write(
                credential.json(
                    exclude_none=True,
                    exclude_unset=True,
...
def start_query_job(
    ...
    print(json.dumps(obj=bulk2.create_query_job(query=query, version=version), indent=2))
